...
__global__ void GPU_fors_sign_throughput_security_level_1_oneblock(uint8_t* sig, uint32_t* indices, uint8_t* sk_seed, uint8_t* pub_seed, uint32_t fors_addr[8], uint8_t* state_seed, uint32_t* lengths) {
    ...
    for (int i = 0; i < FORS_TREE; i++) {
        ...
        sig_index = sig_index + (7 * HASH_DIGEST);
        __syncthreads();
        //!Depth 5[FORS 2-> 1]
        if (threadIdx.x == 0) {
            GPU_set_tree_height(fors_tree_addr, 6);
            GPU_set_tree_index(fors_tree_addr, idx_offset >> 6);
            GPU_tree_thash_2depth(root + (HASH_DIGEST * i), shared_stack, shared_stack + 2 * HASH_DIGEST,
                iternal_pub_seed, fors_tree_addr, iternal_state_seed);
        }
    }

    if (threadIdx.x == 0) {
        uint32_t test_lengths[WOTS_LEN];
        GPU_fors_final_thash(temp, root, iternal_pub_seed, fors_pk_addr, iternal_state_seed);
        GPU_chain_lengths(test_lengths, temp);
        memcpy(lengths + (blockIdx.x * (WOTS_LEN * (SUBTREE_LAYER + 1))), test_lengths, sizeof(uint32_t) * WOTS_LEN);
    }
}
__global__ void GPU_sphincs_tree_hash_security_level_1_oneblock(uint32_t* lengths, uint8_t* sig, uint8_t* sk_seed, uint8_t* pub_seed, uint8_t* state_seed, uint32_t* leaf_idx, uint64_t* tree) {
    uint32_t iternal_layeridx = 0;
    uint32_t iternal_tid = 0;
    ...
    uint32_t first_leaf_idx = sphincs_leaf_idx;
    uint64_t first_tree = sphincs_tree;
    uint32_t sphincs_tree_addr[8] = { 0, };

    //uint32_t sig_store_index = ((iternal_layeridx + 1)) * (WOTS_BYTES + (HASH_DIGEST * TREE_HEIGHT)) - (HASH_DIGEST * TREE_HEIGHT);
    //__shared__ uint8_t shared_stack[HASH_DIGEST * (1 << (TREE_HEIGHT)) * SUBTREE_LAYER];
    iternal_layeridx = threadIdx.x / (1 << TREE_HEIGHT);    // 0~21
    ...
    for (int i = 0; i < iternal_layeridx; i++) {
        sphincs_leaf_idx = (sphincs_tree & ((1 << TREE_HEIGHT) - 1));
        sphincs_tree = sphincs_tree >> TREE_HEIGHT;
    }
    ...
    GPU_set_tree_addr(sphincs_tree_addr, sphincs_tree);

    //GPU_wots_gen_leaf(shared_stack + HASH_DIGEST * shared_idx, iternal_sk_seed, iternal_pub_seed, iternal_state_seed, (sphincs_idx_offset + iternal_tid), sphincs_tree_addr);
    ...
    if (threadIdx.x < (blockDim.x >> 1)) {
        ...
        sphincs_leaf_idx = first_leaf_idx;
        sphincs_tree = first_tree;
        for (int i = 0; i < iternal_layeridx; i++) {
            sphincs_leaf_idx = (sphincs_tree & ((1 << TREE_HEIGHT) - 1));
            sphincs_tree = sphincs_tree >> TREE_HEIGHT;
        }
        ...
        if (((sphincs_leaf_idx >> 1) ^ 0x1) == (threadIdx.x % 4))
    ...
    if (threadIdx.x < (blockDim.x >> 2)) {
        ...
        if (((sphincs_leaf_idx >> 2) ^ 0x1) == (threadIdx.x % 2))
...
//35 * 22
__global__ void GPU_sphincs_wots_sign_security_level_1_oneblock(uint8_t* sig, uint32_t* lengths, uint8_t* sk_seed, uint8_t* pub_seed, uint8_t* state_seed, uint32_t* leaf_idx, uint64_t* tree) {
    ...
    GPU_set_type(sphincs_tree_addr, ADDR_TYPE_HASHTREE);
    GPU_set_layer_addr(sphincs_tree_addr, iternal_layer);
    GPU_set_tree_addr(sphincs_tree_addr, sphincs_tree);
    GPU_copy_subtree_addr(sphincs_wots_addr, sphincs_tree_addr);
    GPU_set_keypair_addr(sphincs_wots_addr, sphincs_leaf_idx);
    GPU_set_chain_addr(sphincs_wots_addr, threadIdx.x % WOTS_LEN);

    GPU_wots_gen_sk(hash_temp, iternal_sk_seed, sphincs_wots_addr);

    GPU_gen_chain(hash_temp, hash_temp, 0, iternal_lengths, iternal_pub_seed, iternal_state_seed, sphincs_wots_addr);
    memcpy(sig + sig_store_index + (HASH_DIGEST * (threadIdx.x % WOTS_LEN)), hash_temp, HASH_DIGEST);
}
int crypto_sign_signature_security_level_1(uint8_t* sig, size_t* siglen, uint8_t* m, size_t mlen, uint8_t* sk, uint32_t msgNum) {
    uint8_t* sk_seed = sk;
    ...
    //! GPU WOTS+ Params set
    uint8_t* gpu_wots_throughput_sig = NULL;
    ...
    cudaMalloc((void**)&gpu_wots_throughput_sig, msgNum * SUBTREE_LAYER * (WOTS_BYTES + TREE_HEIGHT * HASH_DIGEST));
    ...
    for (int i = 0; i < 100; i++) {
        ...
        cudaMemcpy(cpu_wots_sign, gpu_wots_throughput_sig, msgNum * SUBTREE_LAYER * (WOTS_BYTES + TREE_HEIGHT * HASH_DIGEST), cudaMemcpyDeviceToHost);
    }
    ...
    //printf("%d's SPHINCS+ = %4.2f ops\n", msgNum, elapsed_time_ms);
    printf("%d's SPHINCS+ = %4.2f kops\n", msgNum, elapsed_time_ms / 1000);

    /*  getchar();
      for (int i = 0; i < FORS_BYTES; i++) {
          printf("%02X ", cpu_fors_sign[i]);
          if ((i + 1) % 32 == 0)
              printf("\n");
      }
      getchar();
      for (int i = 0; i < SUBTREE_LAYER * (WOTS_BYTES + TREE_HEIGHT * HASH_DIGEST); i++) {
          printf("%02X ", cpu_wots_sign[i]);
          if ((i + 1) % 32 == 0)
              printf("\n");
      }
      getchar();*/
    ...
    cudaFree(gpu_wots_throughput_sig);
    cudaFree(gpu_sk_seed);
    cudaFree(gpu_pub_seed);
    cudaFree(gpu_state_seed);
    cudaFree(gpu_wots_addr);
    cudaFree(gpu_fors_throughput_indices);
    cudaFree(gpu_fors_throughput_lengths);
    cudaFree(gpu_idx_leaf);
    cudaFree(gpu_tree);
    cudaFree(gpu_fors_throughput_test);
    free(cpu_fors_sign);
    free(cpu_wots_sign);
    return 0;
}
...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        crypto_sign_signature_security_level_1(sig, &siglen, m, SK_BYTE, sk, msgNum[i]);
    }
}